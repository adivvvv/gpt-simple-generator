...
try {
    ...
    // Auth-protected API
    if (in_array($path, ['/generate','/seed_keywords','/ideas_seed','/template_plan','/template_bundle'], true)) {
    ...
    } elseif ($method === 'POST' && $path === '/template_plan') {
        Http::requireJson();
        (new \App\Controllers\TemplatePlanController())->handle();
    } elseif ($method === 'POST' && $path === '/template_bundle') {
        Http::requireJson();
        (new \App\Controllers\TemplateBundleController())->handle();
    } else {
