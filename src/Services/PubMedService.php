<?php
// src/Services/PubMedService.php
...
final class PubMedService
{
    ...
    {
        $this->cacheDir = __DIR__ . '/../../' . Env::get('CACHE_DIR', 'storage/cache');
    }
    ...
    private function esearch(string $term, int $retmax): array
    {
        $params = [
            ...
            'retmax' => $retmax,
            'term' => $term,
            'tool' => Env::get('PUBMED_TOOL', 'camelway-generator'),
            'email' => Env::get('PUBMED_EMAIL', ''),
        ];
        if ($k = Env::get('PUBMED_API_KEY', '')) $params['api_key'] = $k;
